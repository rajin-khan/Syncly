...
def list_files_from_all_buckets(query=None):
    ...
    for bucket in bucket_numbers:
        try:
            ...
            for file in files:
                all_files.append((
                    file['name'], file['id'], file.get('mimeType', 'Unknown'),
                    file.get('size', 'Unknown'), file.get('webViewLink', None)
                ))
    ...
    while start_index < total_files:
        ...
        for idx, (name, file_id, mime_type, size, webViewLink) in enumerate(all_files[start_index:start_index+page_size], start=start_index+1):
...
def open_file(all_files):
    ...
    file_name, file_id, mime_type, size, webViewLink = selected_file

    if webViewLink:
        print(f"Opening {file_name} in your web browser...")
        webbrowser.open(webViewLink)
    else:
        print(f"No preview available for {file_name}. You may need to download it.")
