...
async def start(update: Update, context: CallbackContext) -> None:
    ...
    await update.message.reply_text(
        ...
        "/ask - Ask Syncly AI any question about your files or its content\n"
        "/reset - Manually reset Syncly AI's memory (Done automatically after 10 messages)\n"
...
async def ask(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not query:
        await update.message.reply_text("Please provide a question. Usage:\n/ask What is Syncly?")
        return
    ...
    try:
        ...
        if response.ok:
            ...
            await update.message.reply_text("❌ Failed to get a response from the LLM.")
...
async def reset_memory(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if response.ok:
            ...
            await update.message.reply_text("❌ Could not reset memory.")
    except Exception as e:
        await update.message.reply_text(f"⚠️ Error: {e}")
