...
session_memory = defaultdict(list) #memory for LLM context
...
async def llm_ask_endpoint(
    question: str = Body(..., embed=True, description="User's question to the LLM"),
    user_id: Optional[str] = Body("default", embed=True, description="User ID for memory tracking")
):
    ...
    full_messages = [{"role": "system", "content": "You are a helpful assistant in a file management app called Syncly."}] + memory
    ...
    try:
        ...
        session_memory[user_id] = memory[-10:]  # limit to last 10 messages
...
async def reset_llm_memory(user_id: str = Body(..., embed=True)):
