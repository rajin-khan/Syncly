...
async def register_user(user: UserAuth):
    """Register a new user"""
    user_manager = UserReg()
    user_id = user_manager.register_user(user.username, user.password)
    
    if not user_id:
        raise HTTPException(status_code=400, detail="Username already exists")
    
    return {"user_id": str(user_id), "username": user.username}

@app.post("/auth/login", response_model=UserResponse, tags=["Authentication"])
async def login_user(user: UserAuth):
    """Log in an existing user"""
    user_manager = UserReg()
    user_id = user_manager.login_user(user.username, user.password)
